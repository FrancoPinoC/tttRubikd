...
class cubo:
    def __init__(self, xc, yc, zc, mitad_cara, cube_color):
    ...
    def draw(self, sz=None, Tpos=(0, 0, 0), wAng=0, wRot=None,
             angs=(0.0, 0.0, 0.0), rot=None, RGBL=None, exploreRot=False):
        '''
        Tpos means translation position
        wRot: A 3D vector defining the rotation axes, values should be -1, 0 or 1
              0 means the rotation isn't done in that axis, 1 or -1 say it is done there and in which direction
        wAng: Angle of rotation for wRot
        '''
        ...
        if (rot is not None) and not exploreRot:
            for i in range(3):
                if rot[i] != 0:
                    rotAux[i] = rot[i]
                    rotAux[(i + 1) % 3] = 0.0
